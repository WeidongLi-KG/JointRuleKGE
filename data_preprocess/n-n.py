...
tot = (int)(f.readline())
f_n2n = open('n2n.txt','w')
f_rel = open('relation2id.txt','r')
tot_rel = int(f_rel.readline())
for i in range(tot_rel):
    content = f_rel.readline()
    r, rid = content.strip().split()
    rign = rellef[rid] / totlef[rid]
    lefn = relrig[rid] / totrig[rid]
    f_n2n.write("%f\t%f\n" % (lefn, rign))

f_n2n.close()
f_rel.close()
import sys; sys.exit('f_n2n finished')
